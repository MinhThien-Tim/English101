(() => {
  ...
  let state = load();
  let deck=[], index=0, correct=0, wrong=[], answered=false, hints=0, elapsed=0, left=0, mode='vi-en', timer=null;

  function load(){try{const value=JSON.parse(localStorage.getItem(KEY)||'{}');return{faces:Array.isArray(value.faces)?value.faces:[],best:Number.isFinite(value.best)?value.best:null,session:value.session||null}}catch(_){return{faces:[],best:null,session:null}}}
  ...
  function next(){index++;if(index>=deck.length){finish();return}renderQuestion()}
  function finish(){stopTimer();const done=Math.min(deck.length,index+Number(answered)),percent=done?Math.round(correct/done*100):0;state.best=Math.max(state.best||0,percent);state.session=null;save();$('#quizPlay').style.display='none';$('#quizSetup').style.display='none';$('#quizEnd').style.display='block';$('#qFinalScore').textContent=`${correct}/${done}`;$('#qFinalText').textContent=`${wrong.length} mục cần ôn · Điểm tốt nhất ${state.best}%`;$('#qRetryWrong').style.display=wrong.length?'':'none'}
  ...
  function resume(){const s=state.session;if(!s)return;({deck,index,correct,wrong,mode,hints,elapsed,left}=s);$('#qDirection').value=mode;$('#qSeconds').value=String(s.seconds||0);$('#quizPlay').dataset.mode=s.currentMode;$('#quizSetup').style.display='none';$('#quizEnd').style.display='none';$('#quizPlay').style.display='block';renderQuestion(true)}
  ...
  $('#qAnswer').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();answered?next():check()}});
